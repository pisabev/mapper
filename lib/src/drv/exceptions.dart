...
enum PostgreSQLSeverity {
  /// A [PostgreSQLException] with this severity indicates the throwing
  ...
  /// A [PostgreSQLException] with this severity indicates the throwing
  /// connection encountered an error when executing a query and the query
  /// has failed.
  error,
  ...
  /// A [PostgreSQLException] with this severity indicates a failed a
  /// precondition or other error that doesn't originate from the database.
  unknown
}
...
class PostgreSQLException implements Exception {
  ...
  PostgreSQLException._(List<ErrorField> errorFields, {this.stackTrace}) {
    final finder = (identifer) =>
        errorFields.firstWhereOrNull((e) => e.identificationToken == identifer);
    ...
    position = i != null ? int.tryParse(i) : null;
  ...
  /// An index into an executed query string where an error occurred,
  /// if by provided by the database.
  int? position;
